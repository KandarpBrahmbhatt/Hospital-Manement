...
import { getSinglePatientApi, updatePatientApi } from "../../services/patientApi";
...
const EditPatient = () => {
  ...
  useEffect(() => {
    const fetchPatient = async () => {
      if (!id) return;
      try {
        const res = await getSinglePatientApi(id);
        const patientData = res.data;
        if (patientData) {
          // Format expiration date if it exists
          let expDate = "";
          if (patientData.insurance?.validTill) {
            expDate = new Date(patientData.insurance.validTill).toISOString().split("T")[0];
          }
          ...
          setFormData({
            ...
            insurance: {
              ...
              coverageLimit: patientData.insurance?.coverageLimit?.toString() || "",
              validTill: expDate,
            },
          });
        }
        ...
        setError("Failed to retrieve patient file for editing.");
      } finally {
        setFetching(false);
      }
    };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error("Failed to update patient file:", err);
      setError(err.response?.data?.message || "Failed to update patient file. Please try again.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex min-h-screen bg-slate-50 font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="p-6 sm:p-8 space-y-6 max-w-3xl w-full mx-auto">
          {/* Header */}
          <div>
            <h1 className="text-3xl font-bold text-slate-800 tracking-tight">Edit Patient Profile</h1>
            <p className="text-slate-500 mt-1">Modify patient demographics or update coverage details</p>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8">
            ...
            {fetching ? (
              <div className="flex flex-col items-center justify-center py-12 space-y-3">
                ...
                <span className="text-sm font-medium text-slate-500">Loading patient file...</span>
              </div>
            ) : (
